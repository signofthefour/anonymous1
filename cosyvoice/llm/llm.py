...
class Qwen2Encoder(torch.nn.Module):
    ...
    def forward(
        ...
        if not output_hidden_states:
            return outs.hidden_states[-1], masks.unsqueeze(1), None, None
        else:
            return (
    ...
    def forward_one_step(self, xs, masks, cache=None, inference_params=None):
        input_masks = masks[:, -1, :]
        outs = self.model(
            inputs_embeds=xs,
...
class Qwen2EncoderHybrid(Qwen2Encoder):
    def __init__(self, pretrain_path):
...
class Qwen2LM(TransformerLM):
    ...
    def forward(
        ...
        # 4. run lm forward
        lm_output, lm_output_mask, output_hidden_states, output_attentions = self.llm(
            lm_input,
            lm_input_len.to(device),
        ...
        return {
            ...
            "attentions": output_attentions if output_attentions else None,
            "lm_output_mask": lm_output_mask,
            "logits": logits,
        }
    ...
    def inference_wrapper(self, lm_input, sampling, min_len, max_len, uuid):
        if hasattr(self, "vllm"):
            ...
            console.log(
                f"[bold green]VLLM Sampling Parameters:[/bold green] {sampling_params}"
            )
            with self.lock:
    ...
    def inference_bistream(
        ...
        for this_text in text:
            ...
            if prompt_speech_token_emb.size(1) == 0:
                ...
                ) or (len(out_tokens) == 0 and lm_input.size(1) == 1):
