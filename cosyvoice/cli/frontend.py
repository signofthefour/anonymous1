...
try:
    ...
    console.log("failed to import ttsfrd, use WeTextProcessing instead")
...
class CosyVoiceFrontEnd:
    ...
    def __init__(
        ...
        if self.use_ttsfrd:
            ...
            model_dir = os.getenv("MODEL_DIR")
            assert (
                self.frd.initialize(f"{model_dir}/CosyVoice-ttsfrd/resource") is True
    ...
    def text_normalize(
        self, text, split=True, text_frontend=True, long_sequence_generation=False
    ):
        if isinstance(text, Generator):
        ...
        if long_sequence_generation:
            # for long sequence generation, we do not split the text
            text = ["".join(text)]
    ...
    def frontend_zero_shot(
        ...
        if zero_shot_spk_id == "":
            ...
            #     f.write(prompt_text)
            #     f.write("\n")
            #     f.write(" ".join([str(i) for i in speech_token[0].tolist()]))
