...
class CosyVoice:
    ...
    def inference_zero_shot(
        ...
        long_sequence_generation=False,
    ):
        ...
        for i in tqdm(
            self.frontend.text_normalize(
                ...
                long_sequence_generation=long_sequence_generation,
            )
        ):
...
class CosyVoice2(CosyVoice):
    ...
    def __init__(
        ...
        # print("LM to be loaded: ", configs['llm'])
        ...
        # Quantization Logic
        # HARD CODE for CPU inference
        load_jit = False
        load_trt = False

        if load_vllm:
            print("===================================Loading vLLM models...")
...
class CosyVoice2CPU(CosyVoice2):
    def __init__(
        self,
        model_dir,
        load_jit=False,
        ...
        if torch.cuda.is_available() is False and (
            ...
            load_jit, load_trt, fp16 = True, False, True
            logging.warning("no cuda device, set load_jit/load_trt/fp16 to False")
        if prunned is True:
            assert (
                num_layers_after_prunned > 0
            ...
            logging.info("not prunning CosyVoice2 model")
