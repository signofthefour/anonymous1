...
class CosyVoiceModel:
    ...
    def load(self, llm_model, flow_model, hift_model, show_weight_compare=False):
        from rich.console import Console
        from rich.table import Table
        import torch

        def compare_weights(model, ckpt, atol=1e-6):
            console = Console()
            ...
            for name, param in model.state_dict().items():
                if name in ckpt:
                    a = param.detach().to(torch.float32).cuda()
                    b = ckpt[name].detach().to(torch.float32).cuda()
                    same = torch.allclose(a, b, atol=atol)
                    status = "[green]✅[/green]" if same else "[red]❌[/red]"
                    table.add_row(status, name)
                else:
                    table.add_row(
                        "[yellow]⚠️[/yellow]", f"{name} (missing in checkpoint)"
                    )

            console.print(table)

        llm_state_dict = torch.load(llm_model, map_location=self.device)
        if any("mamba" in k for k in llm_state_dict.keys()):
            llm_state_dict = {
                k: v
                for k, v in llm_state_dict.items()
                if "llm.model.model.layers." not in k
            }
        self.llm.load_state_dict(llm_state_dict, strict=True)
        if show_weight_compare:
            compare_weights(self.llm, llm_state_dict)
        ...
        with self.llm_context, torch.cuda.amp.autocast(
            ...
            if isinstance(text, Generator):
                ...
                for i in self.llm.inference(
                    ...
                    #     print("first token time: ", time.time() - start_time)
                    self.tts_speech_token_dict[uuid].append(i)
    ...
    def tts(
        ...
        **kwargs,
        ...
        if stream is True:
            ...
            # deal with all tokens
            p.join()
            # initialize this_uuid if this list is empty
            if len(self.tts_speech_token_dict[this_uuid]) == 0:
                with self.lock:
                    self.tts_speech_token_dict[this_uuid] = [0] * 32
            this_tts_speech_token = torch.tensor(
                self.tts_speech_token_dict[this_uuid]
            ).unsqueeze(dim=0)
            this_tts_speech = self.token2wav(
