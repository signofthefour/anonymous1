...
def load_wav(wav, target_sr):
    # print("Loading wav file: {}".format(wav))
...
def export_cosyvoice2_vllm(model, model_path, device):
    ...
    new_lm_head = torch.nn.Linear(
        in_features=feature_size, out_features=pad_vocab_size, bias=True
    )
    with torch.no_grad():
    ...
    new_codec_embed = torch.nn.Linear(
        in_features=feature_size, out_features=pad_vocab_size
    )
    # embed_tokens
