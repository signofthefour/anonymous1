...
def filter(
    ...
    for sample in data:
        ...
        if isinstance(audio_data, np.ndarray):
            ...
            sample["speech"] = torch.from_numpy(audio_array).unsqueeze(0)
...
def compute_f0(data, sample_rate, hop_size, mode="train"):
    ...
    for sample in data:
        ...
        f0 = pw.stonemask(
            waveform.squeeze(dim=0).numpy().astype("double"), _f0, t, sample_rate
        )
        f0 = F.interpolate(
            torch.from_numpy(f0).view(1, 1, -1),
            size=sample["speech_feat"].shape[0],
            mode="linear",
        ).view(-1)
...
def padding(data, use_spk_embedding, mode="train", gan=False, dpo=False):
    ...
    for sample in data:
        ...
        if dpo is True:
            reject_speech_token = [
                torch.tensor(sample[i]["reject_speech_token"]) for i in order
            ]
            reject_speech_token_len = torch.tensor(
                [i.size(0) for i in reject_speech_token], dtype=torch.int32
            )
            reject_speech_token = pad_sequence(
                reject_speech_token, batch_first=True, padding_value=0
            )
