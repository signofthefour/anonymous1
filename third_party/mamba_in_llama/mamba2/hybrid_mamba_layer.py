...
if USE_CAUSAL_CONV1D:
    try:
        ...
        # Success Log
        console.print(
            "[bold green]✔[/bold green] [bold white]Hardware Acceleration Active:[/bold white] "
            "Using [cyan]causal_conv1d_fn[/cyan] and [cyan]causal_conv1d_update[/cyan]"
        )
        console.print(
            "  [bold blue]ℹ[/bold blue] [italic]Note: This provides a [bold]significant improvement[/bold] in memory throughput and inference speed.[/italic]"
        )
        ...
        console.print(
            Panel(
                "[bold red]✘ Error:[/bold red] Failed to import native Mamba kernels.\n"
                "[yellow]Fallback to standard implementation is active. Performance may be degraded.[/yellow]",
                title="[bold red]Dependency Warning[/bold red]",
                border_style="red",
                expand=False,
            )
        )
...
from mamba2.ssd_combined import mamba_chunk_scan_combined
from mamba2.ssd_combined import mamba_split_conv1d_scan_combined
...
class Mamba2(nn.Module, PyTorchModelHubMixin):
    def __init__(
        ...
        self.d_inner = (
            ...
            else (self.expand * self.d_model) // self.world_size
        )
    ...
    def forward(
        ...
        # TODO: with certain values, A_log might be too large/small and cause inf/nan anyway
        # Fix that with clamping?
        ...
        # d_mlp = (zxbcdt.shape[-1] - 2 * self.d_ssm - 2 * self.d_xb - self.nheads) // 2
        ...
        if self.repeat_kv_before_conv:
            ...
            # minic the GQA
            x = rearrange(
                x, "b l (xb_group dstate) -> b xb_group l dstate", dstate=self.d_state
            )
            x = repeat_kv(x, self.repeat_group)
            # x shape: (bsz, n_group, l, dim)
            B = rearrange(
                B, "b l (xb_group dstate) -> b xb_group l dstate", dstate=self.d_state
            )
        ...
        if causal_conv1d_fn is None or self.activation not in ["silu", "swish"]:
            ...
            temp_values = self.conv1d(xBC.transpose(1, 2)).transpose(1, 2)[
                :, -(self.d_conv - 1) :
            ]
            xBC = self.act(
            ...
            # x_contig = xBC.transpose(1, 2).contiguous()
            # print(f"Input to conv: shape={x_contig.shape}, strides={x_contig.stride()}, ngroups={(self.d_model * self.expand) // self.headdim}")
    ...
    def step(self, hidden_states, conv_state, ssm_state):
        ...
        # minic the GQA
        x = rearrange(
            x, "b (xb_group dstate) -> b xb_group dstate", dstate=self.d_state
        )
        x_reshaped = torch.repeat_interleave(x, dim=1, repeats=self.repeat_group)

        B = rearrange(
            B, "b (xb_group dstate) -> b xb_group dstate", dstate=self.d_state
        )
    ...
    def _get_states_from_cache(
        ...
        if self.layer_idx not in inference_params.key_value_memory_dict:
            ...
            if initialize_states:
                ...
                ssm_state.zero_()
        # console.log(
        #     f"Layer {self.layer_idx} using cache with conv_state shape {conv_state.shape} and ssm_state shape {ssm_state.shape}"
        # )
        return conv_state, ssm_state
